        ...
        while(dapan == "1" || dapan == "2" || dapan == "3" || dapan == "4")
        {
            if(dapan == "1")
            {
                if(CheckHeplCall == true)
                {
                    ...
                    cout << "Bạn đã sử dụng cái này rồi" << endl;
            }
            if(dapan == "2")
            {
                if(CheckTuVanTaiCho)
...
{
    Sleep(200);
    cout << "Câu này quá khó đối với tôi, chúc bạn may mắn lần sau";
}
...
void ThamKhaoYKienKhanGia(Question &Q, int vt, string A[])
{
    Sleep(500);
    srand(time(NULL));
    cout << "Cả 4 phương án đều có tỷ lệ khán giả chọn là 25%";
}
